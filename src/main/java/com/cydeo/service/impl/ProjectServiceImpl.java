...
public class ProjectServiceImpl implements ProjectService {
    ...
    public List<ProjectDTO> listAllProjectDetails() {
        // we are getting authenticated user whoever is log in
        String username = SecurityContextHolder.getContext().getAuthentication().getName();

      UserDTO currentUserDTO=userService.findByUserName(username);// hard coded from security
      User user = userMapper.convertToEntity(currentUserDTO);
      List<Project> list =projectRepository.findAllByAssignedManager(user);

        return list.stream().map(project -> {

            ProjectDTO obj=projectMapper.convertToDto(project);//converte to list

            obj.setUnfinishedTaskCounts(taskService.totalNonCompletedTask(project.getProjectCode()));//set up fields
